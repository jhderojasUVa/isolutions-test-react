...
My path was to select the "one component way" because although it can be done in countless ways, I suppose (wrong or right, usually wrong) that what you are looking for is for the solution about maximizing the items on the grid. If not, I'm sorry, but that's my way.

With more time (I have do it in about two hours) the ux grid will be finished. I have choose to use a SVG and not a CANVAS because I think it's best for drawing on it (I can be wrong).

I wanted to give you three solutions: React, Angular and plain (vanilla) but I really have no time. I'm so sorry.

## Solution

You don't say that I must fetch the data on any URL so I take the JSON format you give me (it's on the stat of the component, as it must be). I have put on the component "external" functions for ordering, maximizing, calcuating the area and all the things that I think are apropos to have outside and not in the component in order to be reusable. They can be in an outside file that can be imported but, on this way, you can see it at a glance (remember, I can be wrong).

There's only one component, remember. It do all. It can be done with several components calling one to other (sending the data, for example, an every component do their own calculations -in the grid, no in the grid for example-), but I don't know if you want to know that I know to do it or not, sorry.

I have "downloaded" a function for converting the XML into JSON but it's not used because you give the JSON, but I find convinient having it in case of need.

About the UI it's a simple one. No bootstrap, no nothing (HTML framework), only an small CSS. It can be done better? Yes.

The documentation on every loop, if, return is on the element. I hope you understand it, if not, please feel free to ask me.

# Documentation from create-react-app from NPM
